...
We perform the ANOVA test on multiple taxonomic level.  
...
| x_mean         | there will be as many of these headers as there are unique values in the mapping file under the category passed with the -c option. Each of these fields will contain the mean frequency/abundance/count of the given OTU for the given sample group.  We have two groups in this workshop (early and late) so we will have two columns |